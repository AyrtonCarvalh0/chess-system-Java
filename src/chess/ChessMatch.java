...
public class ChessMatch {
    ...
    private boolean testCheck(Color color){
        ...
        for(Piece p : opponentPieces){
            ...
            if(mat[kingPos.getRows()][kingPos.getColumns()]){
                return true;
            }
    ...
    private Piece makeMove (Position sourceP, Position targetP){
        ...
        if(p instanceof King && targetP.getColumns() == sourceP.getColumns() +2){
            Position sourceT = new Position(sourceP.getRows(), sourceP.getColumns() +3);
            Position targetT = new Position(sourceP.getRows(), sourceP.getColumns() +1);
        ...
        if(p instanceof King && targetP.getColumns() == sourceP.getColumns() -2){
            Position sourceT = new Position(sourceP.getRows(), sourceP.getColumns() -4);
            Position targetT = new Position(sourceP.getRows(), sourceP.getColumns() -1);
            ChessPiece rook = (ChessPiece) board.RemovePiece(sourceT);
            board.placePiece(rook, targetT);
            rook.increaseMoveCount();
        ...
        if(p instanceof Pawn){
            if(sourceP.getColumns() != targetP.getColumns() && capturedPiece == null){
                Position pawnPos;
                if(p.getColor() == Color.WHITE){
                    pawnPos = new Position(targetP.getRows()+1, targetP.getColumns());
                }
                else{
                    pawnPos = new Position(targetP.getRows()-1, targetP.getColumns());
                }
    ...
    private void undoMove(Position source, Position target, Piece capturedPiece){
        ...
        if(p instanceof Pawn){
            if(source.getColumns() != target.getColumns() && capturedPiece == enPassantVulnerable){
                ...
                Position pawnPos;
                if(p.getColor() == Color.WHITE){
                    pawnPos = new Position(3, target.getColumns());
                }
                else{
                    pawnPos = new Position(4, target.getColumns());
                }
